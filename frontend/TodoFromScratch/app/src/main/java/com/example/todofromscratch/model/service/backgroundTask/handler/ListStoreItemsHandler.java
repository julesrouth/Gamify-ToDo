...
public class ListStoreItemsHandler extends MainHandler {

    private GeneralObserver observer;
    private ListStoreItemsTask task;

    public ListStoreItemsHandler(GeneralObserver observer) {
        super();
    ...
    protected void success(Message msg) {
        StoreItemsList storeItems = (StoreItemsList) msg.getData().getSerializable(ListStoreItemsTask.STORE_ITEMS_KEY);
        Cache.getInstance().setStoreItems(storeItems);
        try {
